<template>
  <div ref="table" :id="id" class="advanced-table">
    ...
    <a-table
      ...
      @expand="onExpand"
    >
      <template slot-scope="text, record, index" :slot="slot" v-for="slot in Slots ">
        <slot :name="slot" v-bind="{text, record, index}"></slot>
      </template>
      <template :slot="slot" v-for="slot in slots">
        <slot :name="slot"></slot>
      </template>
      <template slot-scope="record, index, indent, expanded" :slot="$Slots.expandedRowRender ? 'expandedRowRender' : ''">
        <slot v-bind="{record, index, indent, expanded}" :name="$Slots.expandedRowRender ? 'expandedRowRender' : ''"></slot>
      </template>
    </a-table>
    </a-spin>
  </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      Slots() {
        return Object.keys(this.$Slots).filter(slot => slot !== 'expandedRowRender' && slot !== 'title')
      },
      visibleColumns(){
